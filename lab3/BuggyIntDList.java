...
public class BuggyIntDList extends IntDList {
    ...
    private DNode sortedMerge(DNode d1, DNode d2) {
        ...
        if (d1 == null) {
            ...
            return d1;
        } else if (d1._next == null && d2._next == null) {
            if (d1._val <= d2._val) {
                d1._next = d2;
                d2._prev = d1;
                d1._prev = null;
        ...
        if (d1._val <= d2._val) {
            ...
            d2._next = sortedMerge(d2._next, d1);   // FIXME: Replace this line (if needed). HINT: Step Into(F7) using debugger and try to figure out what it does.
            d2._next._prev = d2;
            d2._prev = null;
            return d2;
        }
