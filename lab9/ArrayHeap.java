...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        ...
        if (node1 != null || node2 != null) {
            if (node1 == null) {
                return index2;
            }  else if (node2 == null || node1.priority() < node2.priority()) {
                return index1;
            }
            return index2;
        }
    ...
    private void bubbleUp(int index) {
        ...
        if (getNode(parentIndex) != null && child.priority() < parent.priority()) {
    ...
    private void bubbleDown(int index) {
        ...
        int minInd = min(index, min(left, right));
        if (index != size() && index != minInd) {
            swap(index, minInd);
            bubbleDown(minInd);
        }
    }
