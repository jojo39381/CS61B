...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        BitSet allNums = new BitSet(size());
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                ...
                _board[i][j] = new Sq(i, j, sequence, fixed, direction, group);
                _board[i][j]._successors = _allSuccessors[i][j][direction];
                _board[i][j]._predecessors = _allSuccessors[i][j][0];
                if (cur == size()) {
                    _board[i][j]._successors = null;
                }
                if (cur == 1) {
                    _board[i][j]._predecessors = null;
    ...
    Model(Model model) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                Sq otherSq = model._board[i][j];
                _board[i][j] = new Sq(otherSq);
                Sq original = model._board[i][j];
                Sq copy = _board[i][j];
                copy._successor = original.successor();
                copy._predecessor = original.predecessor();
                copy._head = original._head;
    ...
    boolean autoconnect() {
        ...
        for (Sq i : _allSquares) {
            if (i.sequenceNum() != 0 && i.successor() == null) {
                if (i.successors() != null) {
                    for (Place suc : i.successors()) {
                        Sq unconnected = get(suc);
                        if (i.connectable(unconnected)) {
                            changed = 1;
                            i.connect(unconnected);
                        }
                    }
                }
            }
    ...
    void solve() {
        ...
        for (int i = 1; i < size(); i++) {
            Sq thisSquare = get(solnNumToPlace(i));
            Sq nextSquare = get(solnNumToPlace(i + 1));
            if (thisSquare.connectable(nextSquare)) {
                thisSquare.connect(nextSquare);
            }
        }
        _unconnected = 0;
    }
    ...
    private int arrowDirection(int x, int y) {
        int direction = 0;
        int soln = _solution[x][y];
        int solnSuc = soln + 1;
        if (soln == size()) {
            direction = 0;

        } else {
            Place suc = solnNumToPlace(solnSuc);
            direction = dirOf(x, y, suc.x, suc.y);
        }
        return direction;
    ...
    final class Sq {
            ...
            if (dirOf(this.x, this.y, s1.x, s1.y) == this._dir) {
                if (s1.predecessor() == null && this.successor() == null) {
                    if (s1.sequenceNum() != 0 && this.sequenceNum() != 0) {
                        if (s1.sequenceNum() != _width * _height) {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() == 0 && s1.sequenceNum() == 0) {
                ...
                int joinGroupNum = joinGroups(sgroup, thisGroup);
                this._head._group = joinGroupNum;
            }
            if (this.sequenceNum() != 0) {
                pointer = s1;
                for (int num = this.sequenceNum() + 1; pointer != null;
                ...
                pointer = this;
                for (int num = s1.sequenceNum() - 1; pointer != null;
            ...
            pointer = this;
            for (pointer._head = this._head; pointer != null;
                 pointer = pointer._successor) {
                pointer._head = this._head;
            }
            return true;
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this._predecessor == null && next._successor == null) {
                    ...
                    pointer = next;
                    for (int group = newGroup(); pointer != null;
                         pointer = pointer._successor) {
                        pointer._group = group;
                        pointer._head = next;
                    }
                }
            } else {
                boolean pHasFixed = hasFixedP(this);
                if (!pHasFixed) {
                    pointer = this;
                    int group = -1;
                ...
                boolean sHasFixed = hasFixed(next);
                if (!sHasFixed) {
                    pointer = next;
                    int group = -1;
        ...
        /** check if square is fixed. takes in NEXT. RETURNS boolean */
        public boolean hasFixed(Sq next) {
            boolean hasFixed = false;
            for (Sq pointer = next; pointer != null;
                 pointer = pointer._successor) {
        ...
        /** check if square is fixed. takes in SQUARE. RETURNS boolean */
        public boolean hasFixedP(Sq square) {
            boolean hasFixed = false;
            for (Sq pointer = square; pointer != null;
                 pointer = pointer._predecessor) {
                if (pointer._hasFixedNum) {
                    hasFixed = true;
                    break;
