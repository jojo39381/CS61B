...
public class MatrixUtils {
     ...
     *
     */
    public static void transposeMatrix(double[][] A) {
        for (int i = 0; i < A.length; i++) {
            for (int j = i; j < A[i].length; j++) {
                double current = A[j][i];
                A[j][i] = A[i][j];
                A[i][j] = current;
            }
        }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        Orientation orientationH = Orientation.HORIZONTAL;
        Orientation orientationV = Orientation.VERTICAL;
